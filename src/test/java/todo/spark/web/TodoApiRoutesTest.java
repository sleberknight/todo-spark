...
class TodoApiRoutesTest {
    ...
    void createTodo_withValidTitle_returns201WithCreatedTodo() throws Exception {
        ...
        var created = Json.GSON.fromJson(response.body(), Todo.class);
    ...
    void createTodo_withBlankTitle_returns400() throws Exception {
        ...
        var error = Json.GSON.fromJson(response.body(), ErrorResponse.class);
        assertThat(error.message()).isEqualTo("title is required");
    }

    @Test
    void getTodo_whenExists_returnsIt() throws Exception {
        var created = createViaApi("get me");

        var response = get(BASE_URL + "/" + created.id());

        assertThat(response.statusCode()).isEqualTo(200);
        var fetched = Json.GSON.fromJson(response.body(), Todo.class);
    ...
    void getTodo_whenMissing_returns404() throws Exception {
        var response = get(BASE_URL + "/999999");
    ...
    void getTodo_whenIdNotNumeric_returns400() throws Exception {
        var response = get(BASE_URL + "/not-a-number");
    ...
    void updateTodo_whenExists_updatesFields() throws Exception {
        var created = createViaApi("original title");

        var response = put(BASE_URL + "/" + created.id(), """
                {"title": "updated title", "description": "updated description"}""");

        assertThat(response.statusCode()).isEqualTo(200);
        var updated = Json.GSON.fromJson(response.body(), Todo.class);
    ...
    void updateTodo_whenMissing_returns404() throws Exception {
        var response = put(BASE_URL + "/999999", """
    ...
    void toggleTodo_flipsCompletionEachCall() throws Exception {
        var created = createViaApi("toggle me");

        var firstToggle = patch(BASE_URL + "/" + created.id() + "/toggle");
        ...
        var secondToggle = patch(BASE_URL + "/" + created.id() + "/toggle");
    ...
    void deleteTodo_whenExists_returns204AndRemovesIt() throws Exception {
        var created = createViaApi("delete me");

        var deleteResponse = delete(BASE_URL + "/" + created.id());
        assertThat(deleteResponse.statusCode()).isEqualTo(204);

        var getResponse = get(BASE_URL + "/" + created.id());
        assertThat(getResponse.statusCode()).isEqualTo(404);
    }

    @Test
    void deleteTodo_whenMissing_returns404() throws Exception {
        var response = delete(BASE_URL + "/999999");
    ...
    void listTodos_filtersByStatus() throws Exception {
        var active = createViaApi("active status filter test");
        var completed = createViaApi("completed status filter test");
        patch(BASE_URL + "/" + completed.id() + "/toggle");

        var activeResponse = get(BASE_URL + "?status=active");
        var completedResponse = get(BASE_URL + "?status=completed");

        var activeTodos = Json.GSON.fromJson(activeResponse.body(), Todo[].class);
        var completedTodos = Json.GSON.fromJson(completedResponse.body(), Todo[].class);
    ...
    void unknownApiPath_returnsJsonNotFound() throws Exception {
        var response = get("http://localhost:" + TEST_PORT + "/api/no-such-path");
    ...
    void deleteCompleted_removesOnlyCompletedTodos() throws Exception {
        var active = createViaApi("stays active");
        var completed = createViaApi("gets bulk-deleted");
        patch(BASE_URL + "/" + completed.id() + "/toggle");

        var response = delete(BASE_URL + "/completed");
        assertThat(response.statusCode()).isEqualTo(200);
    ...
    private static Todo createViaApi(String title) throws IOException, InterruptedException {
        var response = post(BASE_URL, "{\"title\": \"" + title + "\"}");
